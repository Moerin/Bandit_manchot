...
class Bandit(object):
    ...
    def __init__(self, number_arms=0, params=list()):
        self.number_arms = number_arms
        self.params=params

    def recompense(self, index):
        return self.params[index]['gain'] if random.uniform(0.0, 1.0) <= \
    self.params[index]['proba_gain'] else self.params[index]['perte']
...
if __name__ == "__main__":
    params = [{'gain': 10, 'perte':-1.0, 'proba_gain':0.1},
              {'gain': 2, 'perte':0.0, 'proba_gain':0.5}]
    realBandit = Bandit(number_arms=2, params=params)
    bandit = EpsilonGreedyBandit(number_arms=2, epsilon=0.80)
    count = 0
    total_reward = 0.0
    while True:
        for i in xrange(10000):
            ...
            reward = realBandit.recompense(arm.index)
